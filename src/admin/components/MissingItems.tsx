import CardTable from "../../HOC/CardTable";
import { getMissingRentals } from "../../api/missingItems";
import CardContainer from "../../shared/components/UIElements/CardContainer";

export default function MissingItems() {
  return (
    <CardContainer className="col-span-2">
      <CardTable
        headers={{
          name: "Missing Items",
          icon: "data:image/png;base64,",
        }}
        type="table"
        values={getMissingRentals()}
      />
    </CardContainer>
  );
  // return (
  //   <CardContainer className="col-span-2">
  //     <div className="mb-4 flex gap-2 items-center bg-red-400">
  //       <span>
  //         <img src="data:image/png;base64," />
  //       </span>
  //       <h1 className="text-2xl font-bold mb-4 text-gray-700">Rental Tools</h1>
  //     </div>
  //     <MissingItemsComponent
  //       headers={missingItemsHeaders} // Pass updated headers
  //       data={dummy_data}
  //     />
  //   </CardContainer>
  // );
}
